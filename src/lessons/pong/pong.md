...
_Mesteparten av kodingen av Pong skal du gjøre på egen hånd. Men vi
...
resten av leksjonen vil du finne følgende:_
...
_Når man lager nye programmer og spill er det alltid lurt å begynne
...
_Klossen `sprett tilbake ved kanten`{.blockmotion} er veldig enkel å
...
_Nå som ballen spretter fint kan vi lage racketene som spillerene skal
styre. Disse vil være veldig like hverandre, slik at vi kan først
lage den ene racketen og deretter kopiere denne._
...
- Siden ballen nå treffer en vertikal racket i stedet for en
  horisontal vegg må vi også bytte ut tallet `180` i `pek i retning`{.blockmotion}-klossen. Hvilket tall kan du bruke? Prøv deg
...
_Vi har nå laget en enkel versjon av et av de første dataspillene i
