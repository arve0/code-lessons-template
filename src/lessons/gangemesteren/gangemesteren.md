...
- Vi vil nå kombinere gangeklossen med `tilfeldig tall`{.blockoperators}-klossen, men for å få dette til trenger vi en måte å
...
- Prøv å klikke på skriptet for å teste det (siden det ikke starter med et
...
- Om du vil ha en liten utfordring kan du prøve å bruke flere `sett sammen`{.blockoperators}-klosser slik at spørsmålet blir for eksempel `Hva er 9 ganger 6?`.
...
- Vi bruker først en `gjenta`{.blockcontrol}-kloss slik at vi kan få flere
  oppgaver. Legg merke til at vi også sender en `Nytt spørsmål`{.blockevents}-melding hvis svaret er riktig. Hvis svaret er feil
...
Hvis vi ikke husker hvor mye 3 ganger 4 er, kan vi da telle elefanter og finne
ut at svaret er **12**.
...
- For å tegne et rutenett med Hjelpere bruker vi to
  ...
  andre tall i dette skriptet. Prøv i så fall først å forandre på `sett størrelse til 20%`{.blocklooks}-klossen.

- Nå skal vi tegne dette rutenettet hver gang vi lager et nytt spørsmål.
  Klikk på `Læremester`, og legg til en kloss nederst i `Nytt spørsmål`-skriptet:
...
- Om du gir `Hjelper` flere drakter kan du bruke en `neste drakt`{.blocklooks}-kloss i `Tegn hjelper`-skriptet for å få litt variasjon i
...
- Kanskje `Læremester` kan gi litt mer hjelp når man svarer feil? Klarer du
  få henne til å si `Nei, det riktige svaret er større` eller `Nei, det riktige svaret er mindre`?
