...
_Mesteparten av kodingen av Asteroids skal du gjøre selv (og noe har du allerede
...
_I [Lunar Lander](../lunar_lander/lunar_lander.html) laget vi et flott romskip.
Nå skal vi se hvordan vi kan bruke det samme romskipet i dette prosjektet._

Du vet sikkert at du kan _Remikse_ andres Scratch-prosjekter. Du får da
...
- Du kan nå dra romskip-kopien ut fra ryggsekken, og til figurvinduet i det
  ...
  med kontrollen over romskipet å gjøre, for eksempel om du har et `Sjekk landing`-skript trenger vi ikke det i dette spillet.
...
- Vi skal gjøre en liten forandring i hvordan romskipet oppfører seg.
  ...
  tyngdekraft. Slett derfor klossen som modellerer tyngdekraften i `for alltid`{.blockcontrol}-løkken din, `endre [fartY v] med (-0.01)`{.b}.
...
_Romskipet vårt vil snart fly inn i en asteroidesverm, så vi må montere raketter
som kan sprenge unna asteroidene._
...
- Vi vil bruke kloner slik at vi kan skyte flere skudd. Først trenger vi
  ...
  romskipet. Dette siste kan du gjøre med en kombinasjon av `pek i retning`{.blockmotion}, `retning av`{.blocksensing} og
...
- Til slutt vil vi at også skuddene skal kunne forsvinne ut på den ene siden
  ...
  Kopier koden for å _warp'e_ rundt skjermen fra romskipet til skudd-figuren.
...
_Da er det på tide å lage en asteroidesverm. Noe av det som er kult med
...
å bruke kloner i forskjellige størrelser._
...
- Når figuren `starter som klon`{.blockcontrol} vil vi først at den
  ...
  `berører romskipet`{.blocksensing}. Inne i løkken lar du først asteroiden `gå noen steg`{.blockmotion}. Deretter må du teste om asteroiden `berører et skudd`{.blocksensing}. Hvis den gjør det kan du lage asteroiden mindre med en
  kloss som ligner
...
_Du har nå laget en enkel variant av Asteroids. Men prøv å gjøre spillet
...
- Dersom man klarer å skyte ned alle asteroidene burde man komme videre til
  ...
  asteroider som flyr rundt. Lag derfor en variabel `Antall asteroider`{.blockdata} som du øker med 1 når en asteroide `starter som klon`{.blockcontrol}. Deretter må variabelen minke med 1 når klonen slettes.
